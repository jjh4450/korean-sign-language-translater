...
def similiar(want_word,
             main_list):  # API모듈의finder함수에서 받은 리스트 중에서 가장 유사도가 높은 단어를 가진 링크를 반환함 [[단어1, url],[단어2, url]] -> [[단어2, url]]
    greedy = 100
...
def only_korean(str):  # 정규표현식을 이용해 한국어만 추출함
    a = ' '.join(re.compile('[|ㄱ-ㅎ|ㅏ-ㅣ|가-힣]+').findall(str)).split()
    return a


def clean(sentence):  # 형태소를 분리하는 함수
    okt = Okt()
...
def jamo_bunri(word):  # 자음과 모음을 나눔 가->[ㄱ,ㅏ]
